...
const SKIP_PATHS = [
  '/.well-known', '/actor', '/api', '/data', '/favicon', '/feeds.json', '/feedIndex.json',
...
export async function trackHit (req, env) {
  if (!config.analytics || !env.DB) return
  ...
  if (path.startsWith('/rss/') || (path.startsWith('/assets/rss/') && path.endsWith('.xml'))) {
    ...
    await env.DB.prepare(
      'INSERT INTO hits (ts, path, country, city, region, device, referrer, ip_hash, is_bot, asn, rss_feed, rss_subs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).bind(ts, path, cf.country || '?', cf.city || '?', cf.region || '?', 'desktop', '', ipHash, 0, asn, feed, parsed?.subscribers || null).run().catch(() => {})
    return
  }
  ...
  if (kind === 'bot') {
    await env.DB.prepare(
      'INSERT INTO hits (ts, path, country, city, region, device, referrer, ip_hash, is_bot, asn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).bind(ts, path, cf.country || '?', cf.city || '?', cf.region || '?', 'desktop', '', ipHash, 1, asn).run().catch(() => {})
    return
  }
  ...
  await env.DB.prepare(
    'INSERT INTO hits (ts, path, country, city, region, device, referrer, ip_hash, is_bot, asn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  ).bind(ts, path, cf.country || '?', cf.city || '?', cf.region || '?', device, referrer, ipHash, 0, asn).run().catch(() => {})
}
...
export async function handleAnalytics (req, env, hostname) {
  ...
  if (accept.includes('text/html')) {
    return new Response(ANALYTICS_TEMPLATE, { headers: { 'Content-Type': 'text/html;charset=UTF-8' } })
  }
